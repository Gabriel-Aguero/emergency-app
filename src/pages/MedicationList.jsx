    ...
    if (daysUntilExpiration > 40) {
      icon = (
        <FaCheckCircle
          ...
          aria-label="Check icon indicating safe expiration date"
        />
      );
      ...
      icon = (
        <FaExclamationTriangle
          ...
          aria-label="Warning icon indicating approaching expiration date"
        />
      );
    } else {
      icon = (
        <FaTimesCircle
          ...
          aria-label="Danger icon indicating near expiration date"
        />
      );
    }
  ...
  return (
    <>
      {showFormRegisterInfo ? (
        <FormRegisterCart idCarro={medicacionList.id} />
      ) : (
        <>
          {/* aqui recorro la lista de medicamentos para mostrar  */}
          {medicacionList.map((list) => {
            const { icon, daysUntilExpiration } = getIconAndDaysByExpiration(
              list.medExpiration
            );
            return (
              <div key={list.id}>
                <div
                  ...
                  className="border flex w-72 flex-col rounded-lg border-gray-300 dark:border-gray-700 mt-2 mb-2 shadow-md gap-4"
                >
                  ...
                  >
                    <div className="py-2 flex flex-col gap-2 font-medium">
                      <p className="text-gray-500 dark:text-gray-400">
                        Fecha de vencimiento: {list.medExpiration}
                      </p>
                      <p className="text-gray-500 dark:text-gray-400">
                        Número de lote: {list.lot}
                      </p>
                      <p className="text-gray-500 dark:text-gray-400">
                        Cantidad: {list.medQuantity}
                      </p>
                    </div>
          ...
          <ModalRegisterMedicacion
            idCarro={medicacionList.idCarro}
