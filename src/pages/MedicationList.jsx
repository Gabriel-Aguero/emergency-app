...
const MedicacionList = () => {
  ...
  const { deleteMedication,getMedicationByCarro, medications } = useContext(AuthContext);  
  ...
  return (
        <>                                                                   
          <div className="flex flex-col items-center justify-center min-h-screen">
              ...
              <button className="bg-blue-500 mt-5 text-white rounded-md flex gap-2 p-1 hover:bg-blue-700 transition duration-200" onClick={handleAdd}>
                ...
                return (
                  ...
                  <div key={list.id} className="mx-auto p-4 mt-5">
                    <div className="bg-white shadow-md r shadow-slate-800/80 rounded-lg p-4 mb-4 max-w-md transition-all duration-300 hover:shadow-blue-800/60">
                      <h2 className="text-lg font-bold text-gray-800 border-b capitalize">{list.medication}</h2>
                      <div className="mt-2 text-gray-600">
                        <p><strong>Fecha de vencimiento:</strong> {list.medExpiration}</p>
                        <p><strong>Cantidad:</strong> {list.medQuantity}</p>
                        <p><strong>Número de lote:</strong> {list.lot}</p>
                      </div>
